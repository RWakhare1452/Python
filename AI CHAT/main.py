...
def generate_essay_content(details):
    temperature = float(input(Fore.YELLOW + "Do you prefer a more creative and open-ended response (higher temp, e.g., 0.7) or a \
    structured response (lower temp, e.g., 0.2)? Enter temperature value: ")) 
    introduction_prompt = f"Write an introduction for an {details['essay_type']} essay about {details['topic']} on the topic \
    of {details['stance']}."
    introduction = generate_response(introduction_prompt, temperature)
    print(Fore.CYAN + "\n=== Generated Introduction ===")
    print(Fore.GREEN + introduction) 
    body_style = input(Fore.YELLOW + "Would you like the AI to write the body step-by-step or generate a full draft? "
    "(Step-by-step/Full draft): ").lower() 
    if body_style == "full draft":
        ...
        body_step_prompt = f"Write step-by-step arguments for the essay on {details['topic']} with the stance of {details['stance']}. \
        Provide evidence and reasoning for each step."
        body_step = generate_response(body_step_prompt, temperature)
        print(Fore.CYAN + "\n=== Generated Step-by-Step Body ===")
        print(Fore.GREEN + body_step) 
    conclusion_prompt = f"Write a conclusion for an {details['essay_type']} essay about {details['topic']} on the topic of \
    {details['stance']}."
